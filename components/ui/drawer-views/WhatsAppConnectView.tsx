...
export default function WhatsAppConnectView({ appUrl, webUrl, leadSaved }: Props) {
  const { dict } = useDictionary();
  const t = dict.chatbot;
  const whatsappAvailable = Boolean(appUrl || webUrl || SITE_CONFIG.whatsappNumber);

  const qrSrc = SITE_CONFIG.whatsappNumber
    ? `https://api.qrserver.com/v1/create-qr-code/?size=180x180&margin=10&data=${encodeURIComponent(`https://wa.me/${SITE_CONFIG.whatsappNumber}`)}`
    : null;

  return (
    <div className="flex-1 overflow-y-auto px-4 py-4 bg-surface-soft">
      <div className="rounded-2xl border border-border-mid bg-surface-card shadow-sm p-5 flex flex-col gap-3">
        {leadSaved && (
        ...
        {!whatsappAvailable && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-center">
            <p className="text-sm font-semibold text-amber-800">{t.whatsappUnavailableTitle}</p>
            <p className="mt-1 text-xs leading-relaxed text-amber-700">{t.whatsappUnavailableText}</p>
          </div>
        )}
